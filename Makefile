...
CFLAGS         = -Iinclude -Iinc -Isrc -Wall -Wextra \
                 ...
                 -Wl,-z,relro -fomit-frame-pointer -march=native $(EXTRA_CFLAGS)
...
test_driver: test_driver.o parser.o
...
#################
clean:
	rm -f *.o test_driver *.gcda *.gcno *.gcov *.cflow *.png *.dot

distclean: clean
	rm -f *.so parser.so compile_commands.json cscope.files
